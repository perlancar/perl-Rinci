...
Specification has stabilized, I expect no backward incompatible changes for 1.1
series. Feature additions like new properties will still go in.
...
like undo (others like callback/progress report will follow). The specification
is extensible: you can define more properties, or more deps, or more features.
...
A.k.a. L<Perinci::HTTP::Server>. A L<PSGI> application (a set of PSGI
middlewares, really) to serve metadata and function call requests over HTTP,
according to the L<Riap::HTTP> protocol.
...
URI is used as a common syntax to refer to resources. To refer to code entities,
you can use pm (for Perl-based entities), py (Python), php (PHP), rb (Ruby) and
so on. Use the following general syntax to make it language-independent:

 pm:/Package/SubPackage/$var
 py:/Package/SubPackage/func

That is, use the usual C</> (slash) to separate hierarchical packages, and
sigils (a.k.a. variable prefix symbol, for languages that use them) at the final
part of the namespace.

If you refer from the same implementation, the URI scheme should be optional,
e.g. from C<func1> in C<Foo::Bar> Perl module, you can refer to C<func2> in the
same module with just C<func2>, to C<$baz> in C<Foo::Baz> module with
C<../Baz/$var1> or C</Foo/Baz/$var>.
