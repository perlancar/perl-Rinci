...
B<Entities> are elements in your code that can be given metadata. Currently
supported entities are function/method, namespace/package, and variable. Other
entities planned to be supported: class, object, library, application.

B<Entity URIs>. Sometimes entities need to be expressed as URIs, for example as
links or when requesting over Rinci HTTP protocol. To refer to local entities,
use the C<ent> scheme (which can be optional in places). Namespace parts is
separated by C<::>, independent of what the implementation language uses (e.g.
Perl and Ruby use C<::> but Python uses C<.>, all should use C<::> regardless).
...
The name of the package. Useful when aliasing entity (and reusing the metadata)
...
A one-line plaintext description for the link.
