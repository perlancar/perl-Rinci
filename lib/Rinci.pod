...
From DefHash. Float, required. Declare specification version. This property is
required. It should have the value of 1.1. If C<v> is not specified, it is
assumed to be 1.0 and metadata is assumed to be the old, Sub::Spec 1.0.x
metadata.
...
Str. Specify entity version (like package or function version). This is version
as in software implementation version, not to be confused with C<v> which is the
...
From DefHash. Str. Specify default language used in the text properties like
...
From DefHash. Str. The name of the entity. Useful when aliasing entity (and
reusing the metadata) and wanting to find out the canonical/original entity.
...
From DefHash. Str. A one-line summary. It should be plain text without any
...
From DefHash. Array of (str or defhash). A list of tags, useful for
categorization. Can also be used by tools, e.g. L<Perinci::Exporter> in Perl
uses the C<tags> property of the function metadata as export tags.
...
=head3 description (property)

From DefHash. Str. A longer description text. The text should be in marked up in
...
Array of hashes. List to related entities or resources. Can be used to generate
a SEE ALSO and/or LINKS sections in documentation. Each link is a defhash with
the following keys:
...
Str, required. URI is used as a common syntax to refer to resources. If URI
scheme is not specified, tools can assume that it is a C<riap> URI (see
L<Riap>).

=item * caption

From DefHash. Str. A short plaintext title for the link.

=item * description

From DefHash. Str. A longer marked up text description for the link. Suggested
to be formatted to 76 columns.

=item * tags

From DefHash. Array of (str or defhash). Can be used to categorize or select
links. For generating SEE ALSO sections, use the tag 'see'.
...
From DefHash. Any. This property is used to store extended
(application-specific) attributes, much like the C<X-> prefix in HTTP or email
headers. This property can be used as an alternative to using underscore prefix
(e.g. C<_foo>). Some processing tools strip properties/attributes that begin
with underscores, so to pass extended metadata around, it might be more
convenient to use the C<x> property.
