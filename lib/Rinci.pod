...
L<Rias::*> namespace) exist in Perl to do the following:

=over 4

=item * L<Rias::Sub::Wrapper>
...
C<tags> property). Can automatically wrap functions using Rias::Sub::Wrapper
when exporting.

=item * Rias::Sub::To::* modules
...
L<Rias::Sub::BashComplete>).

=item * L<Serabi>

A L<PSGI> application (a set of PSGI middlewares, really) to serve metadata and
function call requests over HTTP, according to the L<Rinci::HTTP> protocol.

=item * L<Rias::Sub::Use>

Use remote packages and import their functions, transparently like you would use
local Perl modules. The remote server can be any Rinci::HTTP-compliant service,
even when implemented in other languages.

=item * C<Rias::Sub::Gen::*>

Since Rinci metadata are just normal data structure, they can be easily
generated. The Rias::Sub::Gen::* Perl modules can generate functions as well as
...
which can add overhead. A single wrapper like Rias::Sub::Wrapper, on the other
...
example, many of the Rias::* Perl modules expect the function metadata to be put
in the C<%SPEC> package variable, with the name of the function as the hash key.
Consult the tool that you are using to find out for sure.
...
L<Rias::Sub::Exporter> in Perl uses the C<tags> property of the function
...
tools, like L<Rias::Sub::CmdLine> in Perl will just dump the content
...
=head2 What does Rinci, Rias mean?

Rinci is taken from Indonesian word B<perincian> or B<rincian>, meaning:
specification, detail.

Rias is also an Indonesian word meaning: to dress up, to decorate, to put on a
make up.
...
C<Sub::Spec> to C<Rinci> (the namespace L<Rias> is now used for the Perl
