...
requests over HTTP, using the L<Rinci::HTTP> protocols. Suitable for creating an
API service.
...
entity and entity action, see the L<Rinci::HTTP> specification):

 http://example.org/api/v1/Package/SubPkg/func
 http://example.org/api/v1/Package/SubPkg/func?-ri-action=get_meta
 https://example.org/Package.SubPkg.$var
 http://example.org/?-ri-entity=ent:&-ri-action=search&-ri-type=function
...
text, like C<description>. The default is C<markdown> and other valid values
include C<none>.
...
tools, like L<Sub::Spec::CmdLine> in Perl will just dump the content
...
specification, each for functions, variables, packages. Bump specification
version from 1.0 to 1.1 due to several incompatibilities like changed C<args>
and C<result> properties, terminologies, defaults. Versioning property (C<v>)
now required.
