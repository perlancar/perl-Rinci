...
=item * B<is_naked> => bool

If set to true, specify that function does not envelope its result. The default
is false, to encourage functions to create envelope. However, wrapper should be
able to create or strip envelope if needed. For example, if you have
"traditional" functions which does not do envelope, you can set this property to
