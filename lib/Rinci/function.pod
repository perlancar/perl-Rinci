...
See the C<partial_arg> feature for more details.
...
and C<-arg_part_len>. They both specify the starting position and length,
whether in character/byte/element depending on the argument schema type. Example
(using Perl):
 ...
 upload_file(name=>"myvideo.mp4", data=>$data1,
             -arg_part_start=>0, -arg_part_len=>10*1024*1024);

 # function usage example: send the next 10MB
 upload_file(name=>"myvideo.mp4", data=>$data2,
             -arg_part_start=>10*1024*1024,
             -arg_part_len=>10*1024*1024);
