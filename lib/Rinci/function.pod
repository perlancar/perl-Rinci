...
=item * B<index_completion> => CODE

A code to supply argument element index completion. Applicable to the following
argument types:
...
A code to supply argument element value completion. Applicable to the following
argument types:
...
Normally in a command-line program, command-line options and arguments will map
to function arguments, e.g. C<--arg 2> will set the C<foo> argument to C<2> and
positional arguments (argument which specifies the C<pos> property and
optionally also a C<greedy> property with true value) will get or slurp
...
=item * C<stdin_or_file>

This means that program should get value from content of file (the name of which
is given from the first remaining command-line argument after all other
arguments/options have been processed), or, if none is
provided, from standard input.

=item * C<stdin_or_files>

This means that program should get value from content of files (the names of
which are taken from the remaining command-line arguments after all other
arguments/options have been processed), or, or if no command-line argument
remains, from standard input. This behavior is similar to the C<< <> >> (diamond
operator) in Perl.

=item * C<stdin_or_args>

This means that program should get value from remaining command-line
argument(s), or if no command-line argument remains, from standard input.

=item * C<stdin_line>

This means that program should get value from a line of standard input; newline
ending will be removed from the argument value.
...
This property specifies argument's dependencies to other arguments (but possibly
to other things too, in the future). This is similar to function's C<deps>
property. It is a hash or dep types and values. The most important dep type is
C<arg> (dependency to another argument). Some dep types are special: C<all>,
C<any>, C<none>. Example:
...
=item * filters => [array of str|code]

Experimental. Filters to apply before argument is converted from text and
validated.
...
=head2 Property: args_rels => hash

This property is used to expression relationships between arguments. The value
is actually L<Sah> schema hash clause set (see hash type in L<Sah::Type>). The
arguments are represented as a hash, and you can use the various Sah clauses to
express relationships between the arguments (hash keys) because the Sah hash
...
Expected result. This should always be in enveloped form.
...
=item * feature: dry_run => bool|hash

If set to a true value, specifies that function supports dry-run (simulation)
mode. Can also be set to a hash like this: C<<{default=>1}>> to mean that
