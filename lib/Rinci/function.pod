...
situation). Keys are alias names, values are itself hashes (alias
specification). Valid alias specification keys: C<summary> (a string, optional),
C<set> (a code, optional, will be given reference to arguments hash and should
set the argument value; if not set, the default behavior is simply to set the
argument value).
