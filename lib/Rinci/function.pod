...
This undo protocol is centered around functions as commands. The responsibility
of doing undo rests on each function.

Not all functions needs to support undo. Those that do must advertise this fact
by setting its C<undo> C<feature> to true.
...
The benefits of this undo system include:

=over 4

=item * Genericity

Any kind of undo operation can be performed, as long as the function supports
it. We can regard function as the black box.

Undo data can be stored and collected anywhere, in memory or elsewhere, by the
caller.

=item * Simplicity

Only caller and function are involved. No undo manager is necessary, unless when
it is.

Standard mechanism (function calling and function arguments) is used, no global
variables or classes involved.

=item * Optional undo

When we do not want/need undo, we just skip passing C<-undo_action> to function.

=back

The drawbacks of this undo system include:

=over 4

=item * Undo data is not transparent/understood outside function

Each function can choose to encode undo data in whatever way it wants.

If a function is upgraded or changed or goes away, undo data can become useless.

A solution for this is to enact a standard/convention for undo data structure so
it is more common and stable. An example of this effort is L<Setup>.

=item * Quality/reliability depends on each function

To keep things under control, more complex function/command should be built on
simpler, well-tested functions/commands.
