...
Specify how to get the value for this argument. 'stdin' means that program
should get value from standard input. 'stdin_or_files' means that program should
get value from files, or if none is specified, from stdin. Other values might be
...
Like C<src>, but only applies when function runs as a command-line program.
