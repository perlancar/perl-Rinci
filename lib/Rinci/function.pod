...
431 (cannot use transaction for this request).

432 (transaction status incorrect). For example when trying to undo a
transaction that is still in progress, or when trying to issue transaction
requests on an aborted transaction.
