...
B<Result envelope>. Any function or method can be given metadata, but the
implementation/tool must make sure that functions return an enveloped result
from your function/method. Result envelope is modeled after HTTP or L<PSGI>
response, it is an array in the following format:
...
wrapper or generator tools to produce it for you.
...
unavailable. Users should try again at a later time.
...
codes that must fit in one unsigned byte (like L<Rias::Sub::CmdLine>).
