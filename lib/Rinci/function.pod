...
C<-tx_action>, C<-action>, C<-confirm/*>, C<-arg_part_start/*>,
C<-arg_part_len/*>, C<-res_part_start>, C<-res_part_len>. They will be explained
in other related sections/documents.
...
checking for range 200-299). See also C<partial_res> feature.
...
element). See the C<partial_arg> feature for more details.
...
=item * feature: partial_arg => bool

If set to true, specifies that function supports receiving only a part of an
argument (think HTTP 1.1 Ranges as analogy). The argument(s) that can be sent
partially must also be marked in the argument specification (by setting the
C<partial> property to true). Useful when argument is potentially very large
(e.g. in a function that receives a large file upload), where partial sending
will avoid loading the whole argument into memory at once.

To send partial argument, you can use the special arguments C<-arg_part_start/*>
and C<-arg_part_len/*>. They both specify the starting position and length, whether
in character/byte/element depending on the argument schema type. Example (using
Perl):
 ...
 # function usage example: send the first 10MB of data
 upload_file(name=>"myvideo.mp4", data=>$data1,
             "-arg_part_start/data"=>0, "-arg_part_len/data"=>10*1024*1024);

 # function usage example: send the next 10MB
 upload_file(name=>"myvideo.mp4", data=>$data2,
             "-arg_part_start/data"=>10*1024*1024,
             "-arg_part_len/data"=>10*1024*1024);

 # function usage example: send the last 7MB (length defaults to data length)
 upload_file(name=>"myvideo.mp4", data=>$data3,
             "-arg_part_start/data"=>10*1024*1024);

=head1 feature: partial_res => bool

If set to true, specifies that function can return partial result.

To request a partial result, you can use special arguments C<-res_part_start>
and C<-res_part_len> similar to when sending partial argument data.

Function will hint the range of data it actually sends via result metadata
properties C<res_part_start> and C<res_part_len>.

An example for a function that reads files (in Perl):
 ...
 # example function usage: request to read first 10M of file content,
 # -result_part_start defaults to 0.
 my $res = read_file(name=>'myvideo.mp4', -res_part_len=>10*1024*1024);
 # => [206, "Partial content", "data...", {-res_part_start=>0, -res_part_len=>10485760}]

 # example function usage: request the next 10MB
 my $res = read_file(name=>'myvideo.mp4', -res_part_start=>10485760, -res_part_len=>10*1024*1024);
 # => [206, "Partial content", "data...", {-res_part_start=>10485760, -res_part_len=>10485760}]

 # example function usage: request the next 10MB, actual returned by server is only ~7MB
 my $res = read_file(name=>'myvideo.mp4', -res_part_start=>20971520, -res_part_len=>10*1024*1024);
 # => [206, "Partial content", "data...", {-res_part_start=>20971520, -res_part_len=>7350518}]
