...
480 codes are related to transaction. 480 is general transaction error, e.g.
transaction status is aborted so further requests for this transaction is
ignored until transaction is aborted.
...
command-line program. 'stdin' means that program should get value from standard
input. 'stdin_or_files' means that program should get value from files, or if
none is specified, from stdin. Other values might be defined in the future.

If argument's type is C<str> or C<array>, the whole standard input and files
will be slurped into memory. If argument's type is C<stream> or C<filehandle>,
program should provide standard input and files as a filehandle (like the
diamond operator in Perl) so function can read input one record at a time.
Record is line, but specifying the record separator should perhaps be possible
in the future.

There should only be one argument with C<src> set to 'stdin' or
'stdin_or_files'.
...
L<Rinci::function::Transaction>. Can be set to a hash containing these keys:
...
Please see Rinci::function::Transaction for more details on transaction.
...
=item * dep: undo_trash_dir => BOOL

If set to 1, specify that function (which should be undoable, i.e. have C<undo>
feature set to true) requires trash directory specifically for undo. Caller
should provide path to trash directory using special argument C<-undo_trash_dir>
only when doing do/undo operation.

Undo trash directory can be provided by transaction manager.
