...
files will be slurped into memory and fed as argument value. If argument
property C<stream> is enabled, framework will pass the argument as a filehandle
so function can read input stream.
