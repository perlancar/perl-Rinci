...
C<-res_part_start>, C<-res_part_len>, C<-arg_stream>. They will be explained in
other related sections/documents.
...
undetermined/infinite length. Currently there must be at most one argument that
has this property set to true. To send value as a stream, caller must set
special argument C<-arg_stream> to true and pass a filehandle or an object that
responds to getline/getitem method. Function can then read data from this
handle.
