...
=item * B<alias_for> => STR

Specify that this argument is actually an alias for another argument. This is
useful when doing argument name completion. Will be explained in the example.
...
Example function metadata and its implementation in Perl:
 ...
 $SPEC{multiply2} = {
     ...
     args => {
         ...
         fp => { # for 'floating point'
             summary => 'Alias for setting round to 0',
             schema => [bool => {is=>1}],
             alias_for => 'round',
         },
     }
 ...
 sub multiply2 {
     ...
     my $round = defined($args{fp}) ? !$args{fp} : $args{round};
...
As mentioned earlier, the 'alias_for' is useful in doing argument name
completion. When none of the arguments are given, completion will list --a, --b,
--round, --noround, --fp. When --fp is already given, the next completion will
list only --a and --b as --fp is an alias for --round.

Another example (demonstrates C<alias_for>):
...
If none of the arguments are given, completion will list --action, --status,
--start, --stop, --restart, and --force. If either --action, --status, --start,
--stop, or --restart is already given, the only completion left for the next
argument will be --force.
