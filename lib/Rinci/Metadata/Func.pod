...
Command line used to produce result, should the function is called from the
command-line (as an alternative to show, for command line help).

=item * http_url => STR

URL used to produce result, should the function is called over HTTP (an
alternative to show in HTTP API documentation).
