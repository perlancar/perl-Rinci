package Rinci::Metadata::Func; # just to make PodWeaver happy
...
=head2 Property: v => FLOAT (required)
...
not as a method). Default is true if unspecified.
...
=head2 Property: name => STR

The name of the function. Useful for generating help/usage information, or when
aliasing functions (and reusing the metadata) and finding out the
canonical/original name of the function.


=head2 Property: summary => STR

A one-line summary. It should be plain text without any markup. It is like the
C<summary> clause in L<Data::Sah>.


=head2 Property: tags => ARRAY OF STR

A list of tags, useful for categorization. Can also be used for exporting tools
like L<Sub::Spec::Exporter> in Perl, where the tags can be used as export tags.


=head2 Property: text_markup => STR
...
=head2 Property: description => STR

A longer description text. The text should be in marked up in format specified
by C<text_markup> and is suggested to be formatted to 80 columns or less. It is
like the C<description> Data::Sah clause.


=head2 Property: see => ARRAY OF STR

A list of references to other functions. It is suggested that this URI format is
used for reference:

 func:foo
 func:Package::SubPkg::bar
 pkg:Package::SubPkg
...
=head2 Property: arg_pass_style => STR (default 'named')
...
Specify function return value. It is a hash containing keys: B<schema> (a Sah
schema to validate the result), B<summary> (like the C<summary> property),
B<description> (like the C<description> property).
...
will suffice. For example, to specify requirement to alternative Perl modules:
 ...
 or => [
     {perl_mod => 'HTTP::Daemon'},
     {perl_mod => 'HTTP::Daemon::SSL'},
 ],
...
=head3 Should I add a new metadata property, or add a new feature name to the C<features> property, or add a new dependency type to the C<deps> property?

If your property describes a dependency to something, it should definitely goes
to as a new dependency type. If your property only describes what the function
can do and does not include any wrapper code, then it probably goes into
C<features>. Otherwise, it should become a new metadata property.
...
doesn't include any wrapper code. On the other hand, C<timeout> is a metadata
property because it involves the wrapping code adding a timeout mechanism (e.g.
an eval() block and alarm() in Perl).
