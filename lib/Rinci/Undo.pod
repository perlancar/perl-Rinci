...
This protocol (I<riundo> for short) is now deprecated in favor of
L<Rinci::Transaction> (I<ritx> for short) for several reasons:
...
data from the that function) to perform the undo. This is sometimes slightly
cumbersome. The undo action might be better provided by other functions
(example: a C<delete_user()> function to undo the C<create_user()>), but we
still have to go through the same function first.
