...
User calls one or more functions using $tm->call(f=>$name, args=>$args). TM will
first check whether function named $foo is indeed eligible to be used in a
transaction (see L</"Function requirements"> for more details on this). If
function is eligible, TM will first call function using C<< -dry_run=>1 >> to
get undo data. TM then records the undo data first in its journal, to be able to
undo the call later (either for rollback in the event of failure, or for undoing
the transaction). After that, TM will call the function again, using C<<
