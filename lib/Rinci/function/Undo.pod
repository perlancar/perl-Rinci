...
also passes C<-tx_manager> and C<-tx_call_id>. Function also gets C<-undo_data>
...
also passes C<-tx_manager> and C<-tx_call_id>. Function also gets C<-undo_data>
from the transaction manager. And then it performs steps like in the do steps.
...
The non-undoable function is also non-transactional. But it can still be
idempotent. And it can manipulate the transactions if it needs too. In the
example, the empty_trash() function instructs the transaction manager to discard
