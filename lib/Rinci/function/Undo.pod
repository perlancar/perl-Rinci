...
If a function is declared to be transactional (C<tx> C<feature> set to true, and
C<idempotent> C<feature> currently also needs to be set to true), the protocol
is different.

B<Performing do>. Aside from L<-undo_action> set to C<do>, the function is
passed C<-tx> special argument (the transaction manager object, described in
L<Rinci::Transaction>). The whole operation and its undo as well must be
comprised of a series of unit steps. Before performing each step, its undo step
must be recorded in the undo journal, which will record the information to a
stable storage:
 ...
 for my $step (@steps) {
     ...
     $res = $tx->record_step(undo_step=>$undo_step);
     if ($res->[0] != 200) {
         $tx->rollback;
         return [500, "Can't record transaction: $res->[0] - $res->[1]"];
     }
     $res = perform_step($step);
     if ($res->[0] != 200) {
         $tx->rollback;
...
also passes C<-tx>. Function gets the list of steps from the transaction manager
object:
...
It should then perform each step, preceded by recording each step's undo step.
On success it should return 200.

B<Performing redo>. To perform redo, caller sets C<-undo_action> to C<redo> and
also passes C<-tx>. Function gets the list of steps from the transaction
manager:
...
When calling function, caller needs to provide appropriate references to
required undo storage in the special argument C<-undo_storage> (or through
C<-tx>). Example:
